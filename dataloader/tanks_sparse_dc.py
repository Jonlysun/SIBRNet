...
class TanksSparse(Dataset):
    ...
    def get_train_dataset(self):
        self._train_Tanks_path = os.path.join(self.root_path, 'Train')
        print('Train dataset {} load…………'.format(self._train_Tanks_path))
        dsets = MultiDataset(name='train')
        for dset in os.listdir(self._train_Tanks_path):
            path = os.path.join(self._train_Tanks_path, dset, "s0.5")
            dsets.append(self.get_sub_set(dset, path, train=True))
        return dsets
    ...
    def get_val_dataset(self):
        ...
        dsets = []
        for dset in config.Tank_scenes:
            path = os.path.join(self._val_Tanks_path, dset, "s0.5")
            dsets.append(self.get_sub_set(dset, path, train=False))
        return dsets
    
    def get_test_dataset(self):
        self._test_Tanks_path = os.path.join(self.root_path, 'Test')
        # self._test_Tanks_path = os.path.join(self.root_path, 'Train')
        print('Test dataset {} load…………'.format(self._test_Tanks_path))
        dsets = []
        for dset in config.Tank_scenes:
            path = os.path.join(self._test_Tanks_path, dset, "s0.5")
            dsets.append(self.get_sub_set(dset, path, train=False))
        return dsets
    
    def get_sub_set(self, dset, scene_path, train):
        ...
        if self.sparse == '4':
            ...
            sparse_dm_paths = sorted(glob.glob(os.path.join(scene_path, 'depths', 'sparse_16', f"im_*.pfm")))
...
class TanksSparse_subset(BaseDataset):
    ...
    def base_getitem(self, idx, rng):
        if self.nbs_mode == 'near':
            ...
            nbs.remove(idx)            
        elif self.nbs_mode == 'near1':
            length = len(self.tgt_im_paths) - 1
            nbs_left = min(self.n_nbs//2, idx//self.step)
            nbs_right = self.n_nbs - nbs_left
            choosed = []
            choosed = [idx - i*self.step for i in range(1, nbs_left+1)] + [idx + i*self.step for i in range(1, nbs_right+1)]
            choosed = [min(max(i,0),length-1) for i in choosed]
            nbs = choosed
        else:
