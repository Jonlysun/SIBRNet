...
class Surround(Dataset):
    ...
    def get_test_dataset(self):
        if self.scale == 0.5:
            self._test_paths = os.path.join(self.root_path, 'Test')
        elif self.scale == 0.25:
            self._test_paths = os.path.join(self.root_path, 'Test_0.25')
        print('Test dataset {} load…………'.format(self._test_paths))
        dsets = []
        for dset in config.Surround_scenes:
            path = os.path.join(self._test_paths, dset)
            dsets.append(self.get_sub_set(dset, path, train=False))
        return dsets
    
    def get_sub_set(self, dset, scene_path, train=True):

        if self.sparse == '4':
            sparse_dm_paths = sorted(glob.glob(os.path.join(scene_path, 'depths', 'sparse_4', f"*.pfm")))
        elif self.sparse == '8':
            sparse_dm_paths = sorted(glob.glob(os.path.join(scene_path, 'depths', 'sparse_8', f"*.pfm")))
        elif self.sparse == '16':
            sparse_dm_paths = sorted(glob.glob(os.path.join(scene_path, 'depths', 'sparse_16', f"*.pfm")))
        elif self.sparse == 'all':
            sparse_dm_paths = sorted(glob.glob(os.path.join(scene_path, 'depths', 'dense_new', f"*.pfm")))
        else:
        ...
        im_paths = sorted(glob.glob(os.path.join(scene_path, 'images', f"*.png")))
        im_paths += sorted(glob.glob(os.path.join(scene_path, 'images', f"*.jpg")))
        im_paths += sorted(glob.glob(os.path.join(scene_path, 'images', f"*.jpeg")))
        dense_dm_paths = sorted(glob.glob(os.path.join(scene_path, 'depths', 'dense_new', f"*.pfm")))
        intrinsics = np.load(os.path.join(scene_path, 'poses', 'dense', 'intrinsics.npy'))
        extrinsics = np.load(os.path.join(scene_path, 'poses', 'dense', 'extrinsics.npy'))
        

        tgt_ind = list(range(len(im_paths)))
        src_ind = list(range(len(im_paths)))

        dset = Surround_subset(
            ...
            tgt_intrinsics=intrinsics[tgt_ind],
            tgt_extrinsics=extrinsics[tgt_ind],
            ...
            src_intrinsics=intrinsics[src_ind],
            src_extrinsics=extrinsics[src_ind],
...
class Surround_subset(BaseDataset):
    def __init__(self, 
                 name,
                 tgt_im_paths,
                 tgt_sparse_dm_paths,
        ...
        self.tgt_extrinsics=tgt_extrinsics
        ...
        self.src_extrinsics=src_extrinsics
        # self.patch = True
    ...
    def base_getitem(self, idx, rng):
        if self.nbs_mode == 'near':
            if self.eval_seq == 'all':
                ...
                nbs.remove(idx)
            elif self.eval_seq == 'sub':
                # tgt_id = config.dtu_interpolation_ind[idx]
                # nbs = config.dtu_interpolation_ind_src[tgt_id]
                tgt_id = config.dtu_extrapolation_ind[idx]
                nbs = config.dtu_extrapolation_ind_src[tgt_id]
        else:
        ...
        tgt_K = self.tgt_intrinsics[idx]
        tgt_extrinsic = self.tgt_extrinsics[idx]
        ...
        src_sparse_depths = np.array([self.load_pad(self.src_sparse_dm_paths[ii]) for ii in nbs])
        # src_sparse_depths = np.resize(src_sparse_depths, (h, w))
        # for ii in nbs:
        # dense_dp = self.load_pad(self.src_dense_dm_paths[0])
        # print(dense_dp.shape)
        # for ii in nbs:
        #     depth_tmp = self.load_pad(self.src_dense_dm_paths[ii])
        #     print(depth_tmp.shape)
        # print(idx)
        ...
        src_Ks = np.array([self.src_intrinsics[ii] for ii in nbs])
        src_extrinsics = np.array([self.src_extrinsics[ii] for ii in nbs])
        ...
        for i in range(self.n_nbs):
            src_extrinsic = src_extrinsics[i][:3, :]
            tgt_extrinsic = tgt_extrinsic[:3, :]
